...
Создание или поиск элемента само по себе ничего нового не показывает. Элемент виден, только если он находится в дереве корня, добавленного на экран одним из методов [UI](../ui/#где-появляется-результат), и его `visible` не отключён. `RemoveFromHierarchy()` скрывает его, сохраняя объект для повторного добавления; `Delete()` удаляет окончательно.
...
`GetChild()` ищет именованного потомка рекурсивно и возвращает `nil`, если совпадения нет. `GetChildAt()` возвращает непосредственного потомка по индексу от `0`.

:::caution[Известная проблема текущей сборки]
`GetChildAt()` выбрасывает Lua-ошибку для индекса вне диапазона вместо возврата `nil`. Перед вызовом проверяйте `index >= 0` и `index < element.childCount`. Проблема передана разработчику и будет исправлена в будущей сборке.
:::
