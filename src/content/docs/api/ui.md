...
:::caution[Известная проблема текущей сборки]
`GetMousePosition()` сейчас возвращает raw screen coordinates, а не координаты UI Toolkit panel. Результат нельзя напрямую использовать для layout или передавать в `UiToViewportPoint()` без собственного преобразования. Проблема передана разработчику и будет исправлена в будущей сборке.
:::
