...
API `Camera` позволяет читать главную камеру сцены и создавать дополнительные камеры. Дополнительная камера рендерит изображение в текстуру: её можно вывести через UI или `ImGui`.
...
`camera:SetActive(true)` запускает рендер дополнительной камеры, но не заменяет изображение главной камеры. Покажите `camera.OutputTexture` через UI-элемент или `ImGui:DrawTexture()`.
