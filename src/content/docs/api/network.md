...
Передача не отображается игроку автоматически. Сервер обычно принимает решение и отправляет клиенту компактное состояние, а клиент уже обновляет [UI](../ui/), [ImGui](../imgui/), звук или уведомление. `SendTable()` также не ждёт ответ и не подтверждает доставку возвращаемым значением.

## Как обрабатывается сообщение

`SendTable()` сериализует поддерживаемые значения из таблицы и передаёт пакет второй части того же Reflex-модуля. При получении её runtime создаёт новую Lua-таблицу и вызывает callback, зарегистрированный через `OnTableReceived()`.

Отправитель не вызывает серверную функцию напрямую и не получает её результат. Если протоколу нужен ответ, принимающая сторона должна отправить отдельное сообщение с собственным `kind` и идентификатором запроса.
...
В обычном сетевом режиме до получателя доходит только первый вызов `SendTable()` за тик, а payload больше `16384` UTF-8 байт молча отбрасывается. Host mode ведёт себя иначе, поэтому успешный локальный тест не гарантирует такое же поведение удалённого клиента. Объединяйте изменения в одну таблицу на тик и держите сообщение меньше лимита.
