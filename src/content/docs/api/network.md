...
`Network` связывает `main.lua` и `server.lua` одного [Reflex-модуля](../../reflex/architecture/). Сообщение представляет собой Lua-таблицу: одна сторона отправляет её через `SendTable()`, другая принимает через `OnTableReceived()`.
...
`nil`, функции и объекты API вроде [`Vector3`](../vector3/) не передаются. Если они находятся в поле таблицы, это поле отсутствует на принимающей стороне.
...
- попытка передать [Vector3](../vector3/), [Agent](../agent/), [Texture](../texture/) или функцию напрямую;
