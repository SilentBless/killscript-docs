...
Это бюджет выполнения Lua VM, а не процент загрузки процессора компьютера. Runtime выдаёт текущему Lua-вызову лимит условных циклов и уменьшает остаток по мере выполнения инструкций. Когда бюджет исчерпан, выполнение может быть отложено или прервано механизмом лимита модуля.

Значения нужны для самоконтроля тяжёлых циклов. Чтение `CpuLimit` ничего не ускоряет и не резервирует дополнительное время.
