...
`Array<T>` — общий адаптер между игровыми коллекциями C# и Lua. Метод вроде `Agents:GetAll()` или `Inventory:GetItems()` формирует коллекцию, а `Array` только даёт Lua-коду безопасно прочитать её длину и элементы. Сам массив ничего не ищет, не обновляет и не применяет к игре.
