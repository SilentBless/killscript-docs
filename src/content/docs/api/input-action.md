...
Не используйте `Disable()` как надёжный способ временно отключить обработчик.
:::
...
Серверная часть Reflex не получает клавиатурный ввод клиента. Обработайте кнопку в `main.lua` и при необходимости передайте состояние на сервер через `Network`.
