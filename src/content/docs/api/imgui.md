...
| API | Область отображения |
|---|---|
| Глобальные `ImGui:Draw*()` | Непосредственно поверх всего игрового экрана. Положение задаёт сам модуль. |
| `ImGui:DrawDebugText()` | Встроенный отладочный блок у края экрана. |
| Методы `ImGuiWindow` | Внутри отдельного HUD-окна, которое игрок может перемещать и масштабировать в редакторе интерфейса. |

Отрисовка выполняется напрямую и не зависит от встроенного HUD-модуля. Все элементы существуют только в кадре, в котором был вызван соответствующий метод.
