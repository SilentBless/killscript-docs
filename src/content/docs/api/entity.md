...
Игровой мир создаёт и удаляет сетевые сущности: снаряды при броске, области взрыва при срабатывании и установленный BridgeCharge после plant. Глобальные API `ThrownProjectiles`, `Explosions` и `PlantedBridgeCharges` запрашивают текущий реестр и оборачивают найденные объекты для Lua.

Обёртка не создаёт сущность и не продлевает её жизнь. После взрыва или despawn сохранённый объект может стать недоступным; для актуального списка снова вызывайте `GetAll()` или профильный метод. На клиенте значения также проходят правила командной видимости, а сервер читает состояние симуляции напрямую.
...
Здесь «взрыв» означает активную игровую сущность эффекта — мгновенный frag, область огня, sonar или защитный купол. Урон и другие эффекты обрабатывает игровая система; чтение объекта только наблюдает уже созданное состояние.
...
Объект появляется после завершения установки предмета `BridgeChargeItem` и удаляется после завершения соответствующего состояния раунда. Он не предоставляет Lua-команд plant/defuse.
...
Позицию и скорость обновляет физика летящего предмета. Lua только читает сетевую сущность; `SimulateTrajectory()` рассчитывает отдельный набор точек и не вмешивается в настоящий полёт.
...
Метод ничего не перемещает и не рисует. Он возвращает снимок рассчитанных точек; для линии в мире передайте их в [`LineRenderer:SetPositions()`](../world-visuals/#setpositions).
