...
`Vector3` сам не является объектом мира. Это значение, которым API передают позицию, направление, скорость или размер. Значение `(1, 0, 0)` может означать направление для `Physics.Raycast()`, позицию точки линии или другой параметр — назначение задаёт принимающий метод.
