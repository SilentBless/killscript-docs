...
## Как обрабатываются объекты

`CreateLineRenderer()` и `CreateSurfaceOverlay()` создают реальные локальные render-объекты в клиентской сцене и возвращают управляющие обёртки. Setter-ы обновляют параметры уже созданного renderer-а; это не команды, которые нужно повторять каждый кадр, если значение не меняется.

Сервер о таких объектах не знает, физика и игровой мир их не учитывают. `RemoveObject()` удаляет только визуальный объект и не воздействует на сущность или поверхность, возле которой он был нарисован.

## Где появляется результат

- `LineRenderer` рисует линию непосредственно в трёхмерном мире по заданным мировым координатам. Это не HUD-элемент.
- `SurfaceOverlay` проецирует цветную область на геометрию мира внутри заданного объёма. Он не создаёт плоское окно поверх экрана.
- Объекты остаются в мире, пока модуль не вызовет `WorldVisuals:RemoveObject()` или не будет выгружен.
- API рисует объекты напрямую и не требует встроенного HUD-модуля. Он также не привязывает их к камере или сущности автоматически — для движущегося маркера обновляйте позицию самостоятельно.
