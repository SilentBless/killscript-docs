...
Анимация видна только тогда, когда `onUpdate` меняет свойства элемента, уже добавленного в отображаемое UI-дерево. `ScheduleAnimation()` сам не выбирает, что анимировать, и не показывает отсоединённый элемент. После `Stop()` элемент остаётся в последнем применённом состоянии; `Complete()` доводит прогресс до конца.
