...
Передача имени из shop-списка не гарантирует корректный subtype-wrapper. Сначала проверьте результат на `nil` и не вызывайте `tostring()` для непроверенного wrapper.
### Методы
